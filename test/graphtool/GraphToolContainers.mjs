...
(async () => {
  ...
  const evaluationResult = await page.evaluate(({ newJson, configFile }) => {
    const ge /* eslint-disable-line no-unused-vars */ = new isg.Graph.Graph(newJson, configFile) // eslint-disable-line no-undef

    // initGraphContainers test

    let visContainerExists

    if (document.getElementById('Graph0_vis_container') !== null) {
      visContainerExists = true
    } else {
      visContainerExists = false
    }

    let optionsContainerExists

    if (document.getElementById('Graph0_options_container') !== null) {
      optionsContainerExists = true
    } else {
      optionsContainerExists = false
    }

    const initGraphContainers = { visContainerExists, optionsContainerExists }

    // colorPicker test

    let colorPickerExists

    if (document.getElementById('Graph0_myDropdown') !== null) {
      colorPickerExists = true
    } else {
      colorPickerExists = false
    }

    const colorPicker = { colorPickerExists }

    // initDeepSearch test

    let deepSearchInputFieldExists

    if (document.getElementById('Graph0_input-field') !== null) {
      deepSearchInputFieldExists = true
    } else {
      deepSearchInputFieldExists = false
    }

    let deepSearchButtonExists

    if (document.getElementById('Graph0_submit-button') !== null) {
      deepSearchButtonExists = true
    } else {
      deepSearchButtonExists = false
    }

    let deepSearchMyCheckboxExists

    if (document.getElementById('Graph0_myCheckbox') !== null) {
      deepSearchMyCheckboxExists = true
    } else {
      deepSearchMyCheckboxExists = false
    }

    const initDeepSearch = { deepSearchInputFieldExists, deepSearchButtonExists, deepSearchMyCheckboxExists }

    return { initGraphContainers, colorPicker, initDeepSearch }
  }, { newJson, configFile })
    .catch(error => {
  ...
  if (evaluationResult !== null) {
    test('Does the graph containers exist', async assert => {
      assert.plan(2)

      assert.ok(evaluationResult.initGraphContainers.visContainerExists)
      assert.ok(evaluationResult.initGraphContainers.optionsContainerExists)
    })

    test('Does the color picker exist', async assert => {
      assert.plan(1)
      assert.ok(evaluationResult.colorPicker.colorPickerExists)
    })

    test('Does the deep search exist', async assert => {
      assert.plan(3)
      assert.ok(evaluationResult.initDeepSearch.deepSearchInputFieldExists)
      assert.ok(evaluationResult.initDeepSearch.deepSearchButtonExists)
      assert.ok(evaluationResult.initDeepSearch.deepSearchMyCheckboxExists)
    })
  }
