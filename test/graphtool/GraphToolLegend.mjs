...
(async () => {
  ...
  const evaluationResult = await page.evaluate(({ newJson, configFile }) => {
    ...
    const setInvisibleLegendGroupsWhiteFunc = ge.graphtool.setInvisibleLegendGroupsWhite(setInvisibleLegendGroupsWhiteInput) // eslint-disable-line no-unused-vars
    ...
    for (const key in legendGroupsExpected) {
      if (key !== 'undefined' && legendGroupsExpected.hasOwnProperty(key)) { // eslint-disable-line no-prototype-builtins
        keysArray.push(key)
      }
    }
    const legendProperties = {}
    const legendPropertiesExpected = {}
    ...
    const nodesExpectedArray = []
    const nodesOutputArray = []
    const edgesExpectedArray = []
    const edgesOutputArray = []
