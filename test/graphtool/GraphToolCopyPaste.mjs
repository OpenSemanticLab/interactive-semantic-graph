...
(async () => {
  ...
  const evaluationResult = await page.evaluate(({ newJson, configFile }) => {
    ...
    // duplicateNode test
    const nodeTodup = ge.graphtool.nodes.get('jsondata/Item:MyProject/budget/0')
    const dupNode = ge.graphtool.duplicateNode(ge.graphtool.nodes.get('jsondata/Item:MyProject/budget/0'))
    dupNode.id = 'jsondata/Item:MyProject/budget/0'

    const duplicateNode = { nodeTodup, dupNode }

    ge.graphtool.network.selectNodes(['jsondata/Item:MyProject/budget/0'])

    const selected = ge.graphtool.network.getSelectedNodes() // eslint-disable-line no-unused-vars
    const copiedNode = ['jsondata/Item:MyProject/budget/0']
    const copiedEdge = ['jsondata/Item:MyProject==HasBudget=>jsondata/Item:MyProject/budget/0']

    ge.graphtool.copiedNodes = copiedNode
    ge.graphtool.copiedEdges = copiedEdge

    ge.graphtool.pasteNodeEdges(copiedNode, copiedEdge)

    const newNode = ge.graphtool.nodes.get('jsondata/Item:MyProject/budget/0/budget/0')

    const pastedToJson = ge.graphtool.drawer.file.jsondata['Item:MyProject'].budget[0].budget[0]

    let newNodeExists = false
    ...
    const jsonExpected = { year: '2023', value: '10' }

    const pasteNodeEdges = { newNodeExists, pastedToJson, jsonExpected }

    return { duplicateNode, pasteNodeEdges }
  }, { newJson, configFile })
    .catch(error => {
  ...
  if (evaluationResult !== null) {
    test('Is the node duplicated correct', async assert => {
      assert.plan(1)
      assert.strictSame(evaluationResult.duplicateNode.nodeTodup, evaluationResult.duplicateNode.dupNode)
    })

    test('Is the node pasted correct', async assert => {
      assert.plan(2)
      assert.ok(evaluationResult.pasteNodeEdges.newNodeExists)
      assert.strictSame(evaluationResult.pasteNodeEdges.pastedToJson, evaluationResult.pasteNodeEdges.jsonExpected)
    })
  }
