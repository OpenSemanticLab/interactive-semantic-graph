...
(async () => {
    ...
    const evaluationResult = await page.evaluate(({ newJson, configFile}) => {
      ...
      const resultColor = ge.drawer.randomHSL();
      const colorPattern = /hsla\(.*?,70%,80%,1\)/;

      const colorObject = ge.drawer.colorObj
      const colorExample = 'hsla(245.72849947349084,70%,80%,1)'
      const property = 'some_literal'

      colorObject[property] = colorExample

      const propertyColor = ge.drawer.registerPropertyColor(property)
      const newPropertyColor = ge.drawer.registerPropertyColor('new_property')

      return { resultColor, colorPattern, propertyColor, colorExample, newPropertyColor };
  }, { newJson, configFile })
  .catch(error => {
  ...
  if (evaluationResult !== null) {
      test('Is the color format hsla', async assert => {
          assert.plan(1);

          assert.match(evaluationResult.resultColor, evaluationResult.colorPattern);
      });

      test('Register property color', async assert => {
        assert.plan(2);

        assert.strictSame(evaluationResult.propertyColor, evaluationResult.colorExample);
        assert.match(evaluationResult.newPropertyColor, evaluationResult.colorPattern);
