...
(async () => {
    ...
    const evaluationResult = await page.evaluate(({ newJson, configFile }) => {
      ...
      // loadStateDefault test
      // const file = new File(['{file:"file", config:"config"}'], "foo.txt", {
      //   type: "text/plain",
      // });
        
      //   let input = {};
      //   input.target = {files: []}
      //   input.target.files.push(file);

      // const loadStateDefaultFunc = ge.graphtool.loadStateDefault(input)
      // const loadStateDefaultResult = {}

      // const loadStateDefault = {loadStateDefaultFunc, loadStateDefaultResult}

      // createLoadStateFunctionality
      // TODO

      // createSaveStateFunctionality test
      const createSaveStateFunctionalityFunc = ge.graphtool.createSaveStateFunctionality()
      const createSaveStateFunctionalityResult = [
        [
            "type",
            "graph_container_id",
            "root_node_objects",
      ...
      const createSaveStateFunctionality = {createSaveStateFunctionalityFunc, createSaveStateFunctionalityResult}

      // loadSaveFunctionality test
      let saveButton;
      if(document.getElementById('Graph0_save')){
        saveButton = true
      }else{
        saveButton = false
      }
      ...
      let loadButton;
        if(document.getElementById('Graph0_load')){
            loadButton = true
        }else{
            loadButton = false
        }

        const loadSaveFunctionality = {saveButton, loadButton}

      return { createSaveStateFunctionality, loadSaveFunctionality, /*loadStateDefault*/ };
  }, { newJson, configFile })
  .catch(error => {
  ...
  if (evaluationResult !== null) {
      test('Does the saved JSON has the right keys', async assert => {
          assert.plan(1);

          assert.strictSame(evaluationResult.createSaveStateFunctionality.createSaveStateFunctionalityFunc, evaluationResult.createSaveStateFunctionality.createSaveStateFunctionalityResult);

      });

        test('Are the load and save buttons created', async assert => {
            assert.plan(2);

            assert.ok(evaluationResult.loadSaveFunctionality.saveButton);
            assert.ok(evaluationResult.loadSaveFunctionality.loadButton);

        })

        // test('Does the loadStateDefault function work', async assert => {
        //     assert.plan(1);
        //     console.log(evaluationResult.loadStateDefault.loadStateDefaultFunc)
        //     assert.strictSame(evaluationResult.loadStateDefault.loadStateDefaultFunc, evaluationResult.loadStateDefault.loadStateDefaultResult);
        // })
  }
