...
(async () => {
    ...
    const evaluationResult = await page.evaluate(({ newJson, configFile }) => {
      ...
      // getAllReachableNodesTo test
      ge.graphtool.deepSearch("10")
      const reachableNodes = ge.graphtool.getAllReachableNodesTo("jsondata/Item:MyProject", [], []);
      const reachableNodesExpected = [
          'jsondata/Item:MyProject',
          'jsondata/Item:SomePerson',
          'jsondata/Item:MyProject/some_literal/0',
          'jsondata/Item:MyProject/some_literal/1',
      ...
      const getAllReachableNodesTo = {reachableNodes, reachableNodesExpected}

      // deleteNodesChildren test
      const nodesToDelete = ge.graphtool.deleteNodesChildren("jsondata/Item:MyProject/budget/0", false);
      const nodesToDeleteExpected = [
        'jsondata/Item:MyProject/budget/0/year',
        'jsondata/Item:MyProject/budget/0/value',
        'jsondata/Item:MyProject/budget/0/budget/0',
        'jsondata/Item:MyProject/budget/0/budget/1',
      ...
      const deleteNodesChildren = {nodesToDelete, nodesToDeleteExpected}

      ge.graphtool.deepSearch("")

      ge.graphtool.expandNodesCleanedUp({nodes: ["jsondata/Item:SomePerson"]})

      // findAllPaths / dfs test

      const allPaths = ge.graphtool.findAllPaths("jsondata/Item:MyProject", "jsondata/Item:MyOtherItem/some_literal");

      const allPathsExpected =  [
           [
             'jsondata/Item:MyProject',
             'jsondata/Item:SomePerson',
             'jsondata/Item:MyOtherItem',
      ...
      const findAllPaths = {allPaths, allPathsExpected}

      // ge.graphtool.expandNodesCleanedUp({nodes: ["jsondata/Item:SomePerson"]})

      // getAllStringsForAllPaths test

      const allStringsForAllPaths = ge.graphtool.getAllStringsForAllPaths(allPaths);

      const allStringsForAllPathsExpected =  [
           [
             'Has Member.HasSomeItem.some_literal',
             'HasOther.HasSomeItem.some_literal'
           ]
         ]

      const getAllStringsForAllPaths = {allStringsForAllPaths, allStringsForAllPathsExpected}

      // getStartAndEndNodesForPath test

      const startAndEndNodesForPath = ge.graphtool.getStartAndEndNodesForPath(["HasBudget"]);
      const startAndEndNodesForPathExpected =  {
        "startNodes": [
            {
        ...
        for(let i = 0; i < startAndEndNodesForPath.startNodes.length; i++) {

            tempArray.push(startAndEndNodesForPath.startNodes[i].id)
            tempArray2.push(startAndEndNodesForPathExpected.startNodes[i].id)
        }

        startAndEndNodesForPath.startNodes = tempArray
        startAndEndNodesForPathExpected.startNodes = tempArray2

        tempArray = []
        tempArray2 = []

        for(let i = 0; i < startAndEndNodesForPath.endNodes.length; i++) {
                
                tempArray.push(startAndEndNodesForPath.endNodes[i].id)
                tempArray2.push(startAndEndNodesForPathExpected.endNodes[i].id)
        }

        startAndEndNodesForPath.endNodes = tempArray
        startAndEndNodesForPathExpected.endNodes = tempArray2

        const getStartAndEndNodesForPath = {startAndEndNodesForPath, startAndEndNodesForPathExpected}

        // comparePaths test

        const comparePathsInput = {path: [
            "HasBudget"
        ], currentPaths: [
            "HasBudget"
        ], pathNodes: [
        ...
        const comparePathsExpected = [
            "jsondata/Item:MyOtherItem",
            "jsondata/Item:MyOtherItem/budget/0"
        ]

        const comparePathsFunc = ge.graphtool.comparePaths(comparePathsInput.path, comparePathsInput.currentPaths, comparePathsInput.pathNodes)

        const comparePaths = {comparePathsFunc, comparePathsExpected}



      return { getAllReachableNodesTo, deleteNodesChildren, findAllPaths, getAllStringsForAllPaths, getStartAndEndNodesForPath, comparePaths };
  }, { newJson, configFile })
  .catch(error => {
  ...
  if (evaluationResult !== null) {
      test('Does getAllReachableNodesTo outputs the right node ids', async assert => {

        assert.plan(1)

        assert.strictSame(evaluationResult.getAllReachableNodesTo.reachableNodes, evaluationResult.getAllReachableNodesTo.reachableNodesExpected)
      });

        test('Does deleteNodesChildren deletes the right nodes', async assert => {
                
             assert.plan(1)
      
             assert.strictSame(evaluationResult.deleteNodesChildren.nodesToDelete, evaluationResult.deleteNodesChildren.nodesToDeleteExpected)
        })

        test('Does findAllPaths finds the right paths', async assert => {

             assert.plan(1)
     
             assert.strictSame(evaluationResult.findAllPaths.allPaths, evaluationResult.findAllPaths.allPathsExpected)
        })

        test('Does getAllStringsForAllPaths finds the right strings', async assert => {
                
                 assert.plan(1)
         
                 assert.strictSame(evaluationResult.getAllStringsForAllPaths.allStringsForAllPaths, evaluationResult.getAllStringsForAllPaths.allStringsForAllPathsExpected)
        })

        test('Does getStartAndEndNodesForPath finds the right nodes', async assert => {
                    
                     assert.plan(1)
             
                     assert.strictSame(evaluationResult.getStartAndEndNodesForPath.startAndEndNodesForPath, evaluationResult.getStartAndEndNodesForPath.startAndEndNodesForPathExpected)
        })

        test('Does comparePaths finds the right nodes', async assert => {
                            
                             assert.plan(1)
                     
                             assert.strictSame(evaluationResult.comparePaths.comparePathsFunc, evaluationResult.comparePaths.comparePathsExpected)
        })
