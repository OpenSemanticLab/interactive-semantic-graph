...
(async () => {
  ...
  const evaluationResult = await page.evaluate(({ newJson, configFile }) => {
    ...
    ge.graphtool.initPopUpHTML()

    const multipleEdges = ge.graphtool.multipleEdgesToSameNode('jsondata/Item:SomePerson')

    const multipleEdgesToSameNode = { multipleEdges }

    return { multipleEdgesToSameNode /* loadSaveFunctionality */ /* loadStateDefault */ }
  }, { newJson, configFile })
    .catch(error => {
  ...
  if (evaluationResult !== null) {
    test('Does the given id has multiple edges', async assert => {
      // console.log(evaluationResult.multipleEdgesToSameNode.multipleEdges)
      assert.plan(1)

      assert.same(evaluationResult.multipleEdgesToSameNode.multipleEdges, true)
    })
  }
