...
function saveEdgeData (data, callback) {
  ...
  if (data !== null) {
    if (this.nodes.get(data.to).manuallyAdded === true) {
      ...
      const oldNode = JSON.parse(JSON.stringify(this.nodes.get(data.to)))
      oldNode.id = newId
      this.nodes.remove(data.to)
      this.nodes.update(oldNode)
      const oldEdge = JSON.parse(JSON.stringify(this.edges.get(data.id)))
      oldEdge.to = newId
...
function addPropertyToJSON (data, mainObject) {
  ...
  const fromNode = mainObject.nodes.get(data.from)
  ...
  if (mainObject.nodes.get(data.to).incomingLabels) {
    const node = mainObject.nodes.get(data.to)
  ...
  if (mainObject.nodes.get(data.to).key === undefined) {
    ...
    const incomingLabels = [data.label]
    ...
    const item = mainObject.nodes.get(data.from).item
    const depth = mainObject.nodes.get(data.from).depth + 1
...
function initPopUpHTML () {
  ...
  const editHtml = '' +
                    ...
                    '</div>' +
                    ''
  const editHtmlDiv = document.createElement('div')
...
function setManipulationOptions (data) {
  this.options.manipulation.deleteNode = function (data, callback) {
    ...
    const nodeGroup = this.nodes.get(data.nodes[0]).group
...
function deleteInJson (data, obj) {
  for (let i = 0; i < data.edges.length; i++) {
    ...
    const fromNode = this.nodes.get(this.edges.get(data.edges[i]).from)

    for (let i = 0; i < fromNode.path.length; i++) {
      finalPlace = finalPlace[fromNode.path[i]]
    }

    const key = this.edges.get(data.edges[i]).objectKey
