...
(async () => {
  ...
  const evaluationResult = await page.evaluate(({ newJson, configFile, params }) => {
    ...
    // ]
    let budget
    ge.graphtool.nodes.get().forEach(element => {
      if (element.label === 'budget') {
        budget = element
      }
    })

    const pos = ge.graphtool.network.getPositions()

    params.pointer.canvas.x = pos[budget.id].x
    params.pointer.canvas.y = pos[budget.id].y

    params.pointer.DOM.x = pos[budget.id].x
    params.pointer.DOM.y = pos[budget.id].y

    ge.graphtool.visOnContext(params)

    const nodes = ge.graphtool.nodes.get()

    const createSaveStateFunctionality = { nodes }

    return { createSaveStateFunctionality /* loadSaveFunctionality */ /* loadStateDefault */ }
  }, { newJson, configFile, params })
    .catch(error => {
  ...
  if (evaluationResult !== null) {
    test('Does the saved JSON has the right keys', async assert => {
      // console.log(evaluationResult.createSaveStateFunctionality.nodes)
      //   assert.plan(1)
