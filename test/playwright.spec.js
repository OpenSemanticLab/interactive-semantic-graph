...
import { fileURLToPath } from 'node:url' // eslint-disable-line
import { join, dirname } from 'node:path' // eslint-disable-line
import * as browsers from 'playwright' // eslint-disable-line
import { node } from 'webpack' // eslint-disable-line
...
test('Visual Comparison Test Demo', async ({ page }) => {
  ...
  const evaluationResult = await page.evaluate(async ({ newJson, configFile }) => { // eslint-disable-line
    const ge = new isg.Graph.Graph(newJson, configFile) // eslint-disable-line
    window.scrollBy(0, 500)

    const nodes = [...ge.graphtool.nodes.get()]

    const position = ge.graphtool.network.getPositions([nodes[0].id])
  ...
  await expect(page).toHaveScreenshot({ maxDiffPixels: 20000 })
