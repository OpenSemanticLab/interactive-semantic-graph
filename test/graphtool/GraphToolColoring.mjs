...
(async () => {
  ...
  const evaluationResult = await page.evaluate(({ newJson, configFile }) => {
    ...
    const nodeColorsAfterFunc = []
    const nodeColorsExpected = []
    ...
    const edgeColorsAfterFunc = []
    const edgeColorsExpected = []
    ...
    const recolorByPropertyNodes = [...ge.graphtool.nodes.get()]
    const recolorByPropertyEdges = [...ge.graphtool.edges.get()]
